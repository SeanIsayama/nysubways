<script>
  ...
  data.forEach((d, i) => {
    const distance = Math.abs(x(d.date) - (mouseX - marginLeft));
  ...
  const width = 1300; // Width of the chart
...
<div class="chart">
  <svg width={width} height={height + 50}>
    ...
    <g stroke="#000" stroke-opacity="0.5">
      ...
      font-family="Nunito, sans-serif" 
      font-size="12px"
      x={data[index] ? x(data[index].date) - 20: 0}
      y={marginTop - 10}
      >
      <tspan>
      ...
      <tspan 
      x={data[index] ? x(data[index].date) - 20: 0} dy="1.2em"
      font-size="9px"
      >
